...
class LightGCN(torch.nn.Module):
    ...
    **recommendation** (via
    ...
    def forward(
        ...
        # 2) CF에 non-linearity추가 (interaction의 형태를 모델이 직접 학습)
        # print('out dim:',out_src.shape, out_dst.shape)
        # out_embedding = torch.concat([out_src, out_dst], dim=1)
        # print(out_embedding.shape)

        # out_mlp = self.fc1(out_embedding)
        # out_mlp = self.activation(out_mlp)
